<div class="flex" style="height: 20px;">
    ...
    <div @class([
        'border border-' . $color . '-600 hover:text-' . $color . '-500' =>
            !$isMyEvent && !$isWidgetEvent,
        'grid grid-cols-7 items-center text-left text-xs font-light cursor-pointer',
        'w-full rounded ml-1 mr-1',
        'hover:bg-' . $color . '-600/20' =>
            $color !== 'secondary' && !$isWidgetEvent,
        'hover:bg-gray-600/20' => $color == 'secondary' && !$isWidgetEvent,
        'text-white hover:text-' . $color . '-500 bg-' . $color . '-500' =>
            $color != 'secondary' && !$isInPast && !$isWidgetEvent && $isMyEvent,
        'text-white hover:text-gray-500 bg-gray-500' =>
            $color == 'secondary' && !$isInPast && !$isWidgetEvent && $isMyEvent,
    ])>
