...
<div>
    <div class="flex gap-4 pb-2 items-center">
        <x-dynamic-component :component="config('timex.pages.buttons.modal.view.time', 'heroicon-o-clock')" class="h-5 w-5" />
        <div>
            ...
            <div class="flex gap-2 items-center text-xs">
                ...
                <div>
                    <x-dynamic-component :component="'heroicon-o-arrow-narrow-right'" class="h-4 w-4" />
                </div>
                <div>
    ...
    @unless (!$event->category && !self::isCategoryModelEnabled())
        <div class="flex gap-4 pb-2 items-center text-xs">
            <div>
                <x-dynamic-component :component="config('timex.pages.buttons.modal.view.category', 'heroicon-o-tag')" class="h-5 w-5" />
            </div>
            <div @class([
    ...
    @unless (!$event->body)
        <div class="flex gap-4 pt-2 items-start">
            <div class="mt-1">
                <x-dynamic-component :component="config('timex.pages.buttons.modal.view.body', 'heroicon-o-annotation')" class="h-5 w-5" />
            </div>
            <div class="pb-2">
    ...
    @unless (!$participants)
        <div class="flex gap-4 items-center pt-2 pb-2">
            <div>
                <x-dynamic-component :component="config('timex.pages.buttons.modal.view.participants', 'heroicon-o-user-group')" class="h-5 w-5" />
            </div>
            <div class="flex gap-2">
                @foreach ($participants as $participant)
                    ...
                    @endphp
                    <div wire:key="{{ rand() }}" class="flex items-center gap-2">
                        <div @class([
    ...
    @unless (!$attachments)
        ...
        <div class="grid grid-cols-1 divide-y border rounded-md mt-2">
            @foreach ($attachments as $attachment)
                <div wire:key="{{ rand() }}" class="p-2 flex justify-between">
                    <div class="flex items-center gap-2">
                        <div>
                            <x-dynamic-component :component="'heroicon-o-paper-clip'" class="h-4 w-4" />
                        </div>
                        <div class="text-sm">
                            {{ $attachment }}
                        </div>
